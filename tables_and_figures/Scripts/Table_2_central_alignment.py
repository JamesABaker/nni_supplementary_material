    ...
    for entry in results:
        if entry == results[0]:
            ...
            def inside_limit_positions(tmh_string, inside_flank_sequence_string):
                ...
                real_lower_value = int(((len(tmh_string)+1) / 2) + len(inside_flank_sequence_string) - 20)
                if real_lower_value < 0:
                    lower_value = 0
                elif real_lower_value >= 0:
                    lower_value = real_lower_value

                real_upper_value = int(((len(tmh_string)+1) / 2) + len(inside_flank_sequence_string) - 10)
            ...
            if "Outside" in str(n_terminal_start):
                ...
                #The inside segment needs lower and upper bounds to be handled sensitively since a negative value will cause nothing/scrambled strings to be recorded. The outside end doesn't really matter if the values are too high; the values recorded will ascend to the maximum value and stop.
                values_of_inside_limit_positions = inside_limit_positions(tmh_sequence, c_flank_sequence)
                #print values_of_inside_limit_positions[0],values_of_inside_limit_positions[1]
                inside_segment =  this_sequence[values_of_inside_limit_positions[0]:values_of_inside_limit_positions[1]]

                #print int(((len(tmh_sequence)+1) / 2) + len(c_flank_sequence) + 10), int(((len(tmh_sequence)+1) / 2) + len(c_flank_sequence) + 20)
                outside_segment = this_sequence[int(((len(tmh_sequence)+1) / 2) + len(c_flank_sequence) + 10):int(((len(tmh_sequence)+1) / 2) + len(c_flank_sequence) + 20)]

            if "Inside" in str(n_terminal_start):
                this_sequence = tmh_sequence_to_use

                values_of_inside_limit_positions = inside_limit_positions(tmh_sequence, n_flank_sequence)
                inside_segment =  this_sequence[values_of_inside_limit_positions[0]:values_of_inside_limit_positions[1]]

                outside_segment = this_sequence[int(((len(tmh_sequence)+1) / 2) + len(n_flank_sequence) + 10):int(((len(tmh_sequence)+1) / 2) + len(n_flank_sequence) + 20)]
